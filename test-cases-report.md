...
| ------ | -------- | ---- | ------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ | ------ | -------- |
| ENV-01 | P1       | API  | Backend starts with valid`.env`            | 1) Create`be/.env` (copy from `be/.env.example` or fill required vars). 2) Ensure `MONGODB_URI` (or `URI`) is set. 3) Ensure `JWT_ACCESS_SECRET` + `JWT_REFRESH_SECRET` are set (or accept defaults). 4) Start backend: `cd be && npm run start:dev`. 5) Wait for “Server is running on port 8080”. | Server starts successfully without crashing;`/api/docs` is reachable.                            |        |          |
