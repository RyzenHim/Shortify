...
1. Start backend (`be/`, port 8080) and frontend (`fe/`, port 3000) with MongoDB running.
2. Work through sections in order for first-time setup; otherwise test by area.
3. Mark **Result** column: `Pass`, `Fail`, or `N/A`.
4. Add notes in **Comments** for failures (screenshot, error message, steps to reproduce).
...
|----|----------|------|-----------|-------|-----------------|--------|----------|
| ENV-01 | P1 | API | Backend starts with valid `.env` | Set `MONGODB_URI`, `JWT_SECRET`, etc.; run `npm run start:dev` in `be/` | Server starts on port 8080 without crash | | |
| ENV-02 | P1 | API | Backend rejects missing MongoDB URI | Remove `MONGODB_URI`/`URI`; start backend | Startup fails with clear env validation error | | |
| ENV-03 | P2 | API | Swagger docs available | Open `http://localhost:8080/api/docs` | Swagger UI loads with auth, urls, admin endpoints | | |
| ENV-04 | P1 | UI | Frontend starts | Run `npm run dev` in `fe/` | App loads at `http://localhost:3000` | | |
| ENV-05 | P2 | API | Health/root endpoint | `GET /api/` | Returns `"Hello World!"` | | |
| ENV-06 | P2 | API | CORS allows frontend origin | Call API from frontend origin | Requests succeed; blocked from unknown origins | | |
